    ...
    flattenLoop(obj, list, pathObj = {}) {
        for (let key in obj) {
            ...
            if (Array.isArray(obj[key])) {
                let arrayObj = this.setObjInArray(key, list, pathList, pathObj, 'array');
                obj[key].forEach((ele, i) => {
                    if (typeof (ele) == 'object') {
                        ...
                        this.setObjInArray(ele, list, newPathList, arrayObj, typeof (ele));
                    }
                });
                delete arrayObj['index'];
            } else if (typeof (obj[key]) == 'object') {
                let arrayObj = this.setObjInArray(key, list, pathList, pathObj, 'object');
                this.flattenLoop(obj[key], list, arrayObj);
            } else {
                this.setObjInArray(key, list, pathList, pathObj, typeof (obj[key]));
            }
        }
    }

    setObjInArray(key, list, pathList, pathObj, type) {
        let arrayObj = {
            "id": this.flattenId,
            "title": key,
            ...
            if (list[i].path.includes('.')) {
                ...
                if (list[i].dataType == 'object') {
                    ...
                    if (list[i].dataType == 'number') {
                        ...
                        obj[list[i].title] = list[i].dataType
                    }
                }
            }
        }
